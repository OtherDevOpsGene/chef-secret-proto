...
# Pull the Azure Key Vault service principal from Chef Vault
spn = chef_vault_item('azure_key_vault', 'spn')
log 'Chef Vault spn' do
  message "spn = #{Chef::JSONCompat.to_json_pretty(spn.to_hash)}"
  level :error
end
